    ...
    return (
        <View style={categoriesStyle.containerShowCategories}>
            ...
            <View style={categoriesStyle.containerActionCategories}>
                <Icon name="caretleft" color={min < 8 ? '#88ccd1' : '#00ACC1'} size={Dimensions.get("window").height / 28} onPress={left} />
                <Text style={categoriesStyle.categoryTextAction}>{max / (8)}/{Math.ceil(categories.length / (max - min))}</Text>
                <Icon name="caretright" color={max >= categories.length ? '#88ccd1' : '#00ACC1'} size={Dimensions.get("window").height / 28} onPress={rigth} />
            </View>
        </View>
    )
}
